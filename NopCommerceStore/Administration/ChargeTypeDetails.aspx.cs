...
{
    public partial class ChargeTypeDetails : BaseNopAdministrationPage
    {
        /// <summary>
        ...
        protected void BindData()
        {
            ...
            if (chargeType != null)
            {
                ...
                this.chkStatus.Checked = chargeType.IsActive;
            }
        }
        ...
        /// </summary>
        /// <returns>
        /// true if action is allow; otherwise false
        /// </returns>
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                {
                    Save();
                    Response.Redirect(string.Format("ChargeTypes.aspx?ChargeTypeID={0}", ChargeTypeID));
                }
                catch (Exception exc)
                {
        ...
        protected void SaveAndStayButton_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                {
                    Save();
                    Response.Redirect("ChargeTypeDetails.aspx?ChargeTypeID=" + ChargeTypeID.ToString());
                }
                catch (Exception exc)
                {
                    ProcessException(exc);
        ...
        protected void Save()
        {
            var chargeType = this.ChargeTypeService.GetChargeType(ChargeTypeID);
            if (chargeType != null)
            {
                chargeType.CreatedOn = chargeType.CreatedOn;
                ...
                if (!string.IsNullOrEmpty(txtChargeName.Text))
                {
                    ...
                    this.ChargeTypeService.UpdateChargeType(chargeType);
                }
            }
        }
