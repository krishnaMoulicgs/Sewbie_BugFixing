...
namespace NopSolutions.NopCommerce.BusinessLogic.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        /// <param name="targetSize">The target picture size (longest side)</param>
        /// <returns></returns>
        private Size CalculateWideDimensions(Size originalSize, int targetSize)
        {
            var newSize = new Size();
        ...
        /// <param name="targetSize">The target picture size (longest side)</param>
        /// <returns></returns>
        private Size CalculateLongDimensions(Size originalSize, int targetSize)
        {
            var newSize = new Size();
            ...
            else
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    if (!File.Exists(Path.Combine(LocalThumbImagePath, fname)))
                    {
                        ...
                        var newSize = CalculateWideDimensions(b.Size, targetSize);
        ...
        /// <param name="targetSize"></param>
        /// <returns></returns>
        public string GetLongPictureUrl(Picture picture, int targetSize)
        {
            return GetLongPictureUrl(picture, targetSize, true);
        }
            ...
            bool showDefaultPicture)
        {
            ...
            lock (s_lock)
            {
                ...
                {
                    localFilename = string.Format("{0}_{1}_w.{2}", picture.PictureId.ToString("0000000"), targetSize, lastPart);
                    if (!File.Exists(Path.Combine(this.LocalThumbImagePath, localFilename)))
                    {
                        if (!System.IO.Directory.Exists(this.LocalThumbImagePath))
                        {
        ...
        /// <summary>
        /// Get picture URL in the long format.
        /// </summary>
        /// <param name="picture"></param>
        /// <param name="targetSize"></param>
            ...
            bool showDefaultPicture)
        {
            ...
            lock (s_lock)
            {
                ...
                {
                    localFilename = string.Format("{0}_{1}_l.{2}", picture.PictureId.ToString("0000000"), targetSize, lastPart);
                    if (!File.Exists(Path.Combine(this.LocalThumbImagePath, localFilename)))
                    {
                        if (!System.IO.Directory.Exists(this.LocalThumbImagePath))
                        {
                        ...
                        using (MemoryStream stream = new MemoryStream(picture.LoadPictureBinary()))
                        {
                            ...
                            var newSize = CalculateLongDimensions(b.Size, targetSize);
        ...
        public byte[] ValidatePicture(byte[] pictureBinary, string mimeType)
        {
            using (MemoryStream stream = new MemoryStream(pictureBinary))
            {
                ...
                {
                    var newSize = CalculateWideDimensions(b.Size, maxSize);
                    var newBitMap = new Bitmap(newSize.Width, newSize.Height);
                    var g = Graphics.FromImage(newBitMap);
                    g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighQuality;
                    g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
