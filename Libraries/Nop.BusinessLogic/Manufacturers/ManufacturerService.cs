...
namespace NopSolutions.NopCommerce.BusinessLogic.Manufacturers
{
    ...
    public partial class ManufacturerService : IManufacturerService
    {
        ...
        private const string MANUFACTURERS_BY_NAME_KEY = "Nop.manufacturer.name-{0}";
        ...
        public Manufacturer GetManufacturerByName(string manufacturerName)
        {
            ...
            string key = string.Format(MANUFACTURERS_BY_NAME_KEY, manufacturerName);
            object obj2 = _cacheManager.Get(key);
            if (this.ManufacturersCacheEnabled && (obj2 != null))
            {
                return (Manufacturer)obj2;
            ...
            var query = from m in _context.Manufacturers
                        where m.Name == manufacturerName && m.Deleted == false
                        select m;
            var manufacturer = query.SingleOrDefault();
        ...
        /// <summary>
        /// Gets a manufacturer by the trimmed 
        /// name. Created for the URL routing 
        /// that we are creating for seller pages.
        /// </summary>
        /// <param name="manufacturerId">Manufacturer identifier</param>
        /// <returns>Manufacturer</returns>
        public Manufacturer GetManufacturerByTrimName(string manufacturerName)
        {
            if (manufacturerName == string.Empty)
                return null;
            ...
            var query = from m in _context.Manufacturers
                        where 
                            CommonHelper.RemoveSpecialCharacters(m.Name) == CommonHelper.RemoveSpecialCharacters(manufacturerName) 
                            && m.Deleted == false
                        select m;
            var manufacturer = query.SingleOrDefault();
