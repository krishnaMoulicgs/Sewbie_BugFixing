...
namespace NopSolutions.NopCommerce.BusinessLogic.Messages
{
    ...
    public partial class MessageService : IMessageService
    {
        ...
        private string ProductListToHtmlTable(Order order, int languageId)
        {
            ...
            string color1 = IoC.Resolve<ISettingManager>().GetSettingValue("MessageTemplate.Color1", "#000000");
            string color2 = IoC.Resolve<ISettingManager>().GetSettingValue("MessageTemplate.Color2", "#ffffff");
            string color3 = IoC.Resolve<ISettingManager>().GetSettingValue("MessageTemplate.Color3", "#ffffff");
           
            #region Products
            sb.AppendLine(string.Format("<tr style=\"background-color:{0};text-align:center;color:{1};\">", color1, color2));
            ...
            //subtotal
            sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, localizationManager.GetLocaleResourceString("Order.Sub-Total", languageId), cusSubTotal));
            
            //discount (applied to order subtotal)
            if (dislaySubTotalDiscount)
            {
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, localizationManager.GetLocaleResourceString("Order.Discount", languageId), cusSubTotalDiscount));
            }
            ...
            if (dislayShipping)
            {
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, localizationManager.GetLocaleResourceString("Order.Shipping", languageId), cusShipTotal));
            }
            ...
            if (displayPaymentMethodFee)
            {
                ...
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, paymentMethodFeeTitle, cusPaymentMethodAdditionalFee));
            }

            //tax
            if (displayTax)
            {
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, localizationManager.GetLocaleResourceString("Order.Tax", languageId), cusTaxTotal));
            }
            if (displayTaxRates)
            {
                foreach (var item in taxRates)
                {
                    ...
                    sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, taxRate, taxValue));
                }
            }
            ...
            if (dislayDiscount)
            {
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, localizationManager.GetLocaleResourceString("Order.Discount", languageId), cusDiscount));
            }
            ...
            foreach (var giftCardUsageHistory in gcuhC)
            {
                ...
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, giftCardText, giftCardAmount));
            }
            ...
            if (order.RedeemedRewardPoints != null)
            {
                ...
                sb.AppendLine(string.Format("<tr style=\"text-align:right;\"><td>&nbsp;</td><td colspan=\"2\" style=\"background-color: {0};padding:0.6em 0.4 em;\">{1}</td> <td style=\"background-color: {0};padding:0.6em 0.4 em;\">{2}</td></tr>", color3, rpTitle, rpAmount));
            }
            ...
            string template, int languageId)
        {
            ...
            ProductVariant pv = IoC.Resolve<ProductService>().GetProductVariantById(order.OrderProductVariants[0].ProductVariantId);

            tokens.Add("Vendor.Name",  HttpUtility.HtmlEncode(pv.Vendor.CompanyName));
            //tokens.Add("Vendor.URL", pv.Vendor.);
            tokens.Add("Vendor.Email",  HttpUtility.HtmlEncode(pv.Vendor.PaypalEmailAddress));

            //TODO: This is a cheat.  Only taking the first note, this may have
            //      consequences in the case where the customers note is not the 
            //      first note in the sequence and a susequent email is sent.
            //      mind the gap.
            if (order.OrderNotes.Count > 0)
            {
                tokens.Add("Order.Note", order.OrderNotes[0].Note);
            }
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public int SendOrderPlacedSellerNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException("order");
            ...
            string email = pv.Vendor.PaypalEmailAddress;

            string subject = ReplaceMessageTemplateTokens(order, localizedMessageTemplate.Subject, languageId);
            string body = ReplaceMessageTemplateTokens(order, localizedMessageTemplate.Body, languageId);
            string bcc = localizedMessageTemplate.BccEmailAddresses;
            var from = new MailAddress(email, pv.Vendor.CompanyName);
            var to = new MailAddress(email, pv.Vendor.CompanyName);
            var queuedEmail = InsertQueuedEmail(5, from, to, string.Empty, bcc, subject, body,
                DateTime.UtcNow, 0, null, pv.Vendor.CustomerId);
            return queuedEmail.QueuedEmailId;
        }

        /// <summary>
        /// Sends an order placed notification to a sewbie admin
        /// </summary>
        /// <param name="order">Order instance</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public int SendOrderPlacedSewbieNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException("order");
        ...
        public string[] GetListOfAllowedTokens()
        {
            ...
            allowedTokens.Add("%Vendor.Name%");
            allowedTokens.Add("%Vendor.Email%");

            allowedTokens.Add("%Order.Note%");
