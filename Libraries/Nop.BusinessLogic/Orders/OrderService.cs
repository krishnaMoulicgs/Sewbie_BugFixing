...
namespace NopSolutions.NopCommerce.BusinessLogic.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        /// <param name="ss">Order shippment status; null to load all orders</param>
        /// <returns>Order collection</returns>
        public List<Order> SearchVendorOrders(DateTime? startTime, DateTime? endTime,
            string customerEmail, int vendorId, OrderStatusEnum? os, PaymentStatusEnum? ps,
            ShippingStatusEnum? ss)
        {
            return SearchVendorOrders(startTime, endTime,
             customerEmail, os, ps, ss, string.Empty, vendorId);
        }
        ...
        public List<Order> SearchVendorOrders(DateTime? startTime, DateTime? endTime,
            string customerEmail, OrderStatusEnum? os, PaymentStatusEnum? ps,
            ShippingStatusEnum? ss, string orderGuid, int vendorID)
        {
            int? orderStatusId = null;
            if (os.HasValue)
                orderStatusId = (int)os.Value;
            ...
            var query = from o in _context.Orders
                        join opv in _context.OrderProductVariants on o.OrderId equals opv.OrderId
            ...
            int recordsToReturn, int orderBy)
        {
            ...
            var report = _context.Sp_SalesBestSellersReport(lastDays,
                recordsToReturn, orderBy, NopContext.Current.User.Vendor.CustomerId).ToList();
            return report;
        }

        /// <summary>
        /// Get the bests sellers report for the vendor
        /// </summary>
        /// <param name="lastDays">Last number of days</param>
        /// <param name="recordsToReturn">Number of products to return</param>
        /// <param name="orderBy">1 - order by total count, 2 - Order by total amount</param>
        /// <returns>Result</returns>
        public List<BestSellersReportLine> VendorBestSellersReport(int lastDays,
            int recordsToReturn, int orderBy)
        {

            var report = _context.Sp_SalesBestSellersReport(lastDays, recordsToReturn, orderBy, NopContext.Current.User.Vendor.CustomerId).ToList();
        ...
        /// <param name="os">Order status</param>
        /// <returns>Result</returns>
        public OrderAverageReportLineSummary VendorOrderAverageReport(OrderStatusEnum os)
        {
            var item = new OrderAverageReportLineSummary();
            item.OrderStatus = os;
            ...
            if (!timeZone.IsInvalidTime(t1))
            {
                DateTime? startTime1 = DateTimeHelper.ConvertToUtcTime(t1, timeZone);
                DateTime? endTime1 = null;
                var todayResult = GetVendorOrderAverageReportLine(os, startTime1, endTime1);
                item.SumTodayOrders = todayResult.SumOrders;
                item.CountTodayOrders = todayResult.CountOrders;
            }
            //week
            ...
            if (!timeZone.IsInvalidTime(t2))
            {
                DateTime? startTime2 = DateTimeHelper.ConvertToUtcTime(t2, timeZone);
                DateTime? endTime2 = null;
                var weekResult = GetVendorOrderAverageReportLine(os, startTime2, endTime2);
                item.SumThisWeekOrders = weekResult.SumOrders;
                item.CountThisWeekOrders = weekResult.CountOrders;
            }
            //month
            ...
            if (!timeZone.IsInvalidTime(t3))
            {
                DateTime? startTime3 = DateTimeHelper.ConvertToUtcTime(t3, timeZone);
                DateTime? endTime3 = null;
                var monthResult = GetVendorOrderAverageReportLine(os, startTime3, endTime3);
                item.SumThisMonthOrders = monthResult.SumOrders;
                item.CountThisMonthOrders = monthResult.CountOrders;
            }
            //year
            ...
            if (!timeZone.IsInvalidTime(t4))
            {
                DateTime? startTime4 = DateTimeHelper.ConvertToUtcTime(t4, timeZone);
                DateTime? endTime4 = null;
                var yearResult = GetVendorOrderAverageReportLine(os, startTime4, endTime4);
                item.SumThisYearOrders = yearResult.SumOrders;
                item.CountThisYearOrders = yearResult.CountOrders;
            }
            //all time
            DateTime? startTime5 = null;
            DateTime? endTime5 = null;
            var allTimeResult = GetVendorOrderAverageReportLine(os, startTime5, endTime5);
            item.SumAllTimeOrders = allTimeResult.SumOrders;
            item.CountAllTimeOrders = allTimeResult.CountOrders;
        ...
        /// <summary>
        /// Places a VENDOR order
        /// </summary>
        /// <param name="paymentInfo">Payment info</param>
        /// <param name="customer">Customer</param>
        /// <param name="orderId">Order identifier</param>
        /// <returns>The error status, or String.Empty if no errors</returns>
        public string PlaceOrder(PaymentInfo paymentInfo, Customer customer, int vendorId, string orderNote,
            out int orderId)
        {
            var orderGuid = Guid.NewGuid();
            return PlaceOrder(paymentInfo, customer, orderGuid, vendorId, orderNote, out orderId);
        }
        ...
        {
            //TODO: Error is occuring in this METHOD 11.18.13 - NFF
            orderId = 0;
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(processPaymentResult.Error))
                {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        int orderPlacedSellerNotificationQueuedEmailId = messageService.SendOrderPlacedSellerNotification(order, localizationManager.DefaultAdminLanguage.LanguageId);
                        if (orderPlacedSellerNotificationQueuedEmailId > 0)
                        {
                            InsertOrderNote(order.OrderId, string.Format("\"Order placed\" email (to seller) has been queued. Queued email identifier: {0}.", orderPlacedSellerNotificationQueuedEmailId), false, DateTime.UtcNow);
                        }

                        int orderPlacedSewbieNotificationQueuedEmailId = messageService.SendOrderPlacedSewbieNotification(order, localizationManager.DefaultAdminLanguage.LanguageId);
                        if (orderPlacedSewbieNotificationQueuedEmailId > 0)
                        {
                            InsertOrderNote(order.OrderId, string.Format("\"Order placed\" email (to store owner) has been queued. Queued email identifier: {0}.", orderPlacedSewbieNotificationQueuedEmailId), false, DateTime.UtcNow);
                        }
            ...
            if (!String.IsNullOrEmpty(processPaymentResult.Error))
            {
                ...
                logService.InsertLog(LogTypeEnum.OrderError, outputPaymentInfoObject(paymentInfo),string.Empty);
                logService.InsertLog(LogTypeEnum.OrderError, customer.FirstName + " " + customer.LastName + " " + customer.CustomerId, string.Empty); 
                logService.InsertLog(LogTypeEnum.OrderError, orderGuid.ToString(), string.Empty);
                logService.InsertLog(LogTypeEnum.OrderError, orderId.ToString(), string.Empty);
            }
            return processPaymentResult.Error;
        }

        private string outputPaymentInfoObject(PaymentInfo pinfo)
        {
            string result = string.Empty;

            result = result + " " + pinfo.BillingAddress.ToString();
            result = result + " " + pinfo.BillingAddress.FirstName.ToString();
            result = result + " " + pinfo.BillingAddress.LastName.ToString();
            result = result + " " + pinfo.BillingAddress.Address1.ToString();
            result = result + " " + pinfo.BillingAddress.Address2.ToString();
            result = result + " " + pinfo.BillingAddress.City.ToString();
            result = result + " " + pinfo.BillingAddress.StateProvince.ToString();
            result = result + " " + pinfo.BillingAddress.ZipPostalCode.ToString();
            result = result + " " + pinfo.ToString();
            result = result + " " + pinfo.PaypalToken.ToString();
            result = result + " " + pinfo.PaypalPayerId.ToString();
            result = result + " " + pinfo.OrderTotal.ToString();
            result = result + " " + pinfo.ShippingAddress.FirstName.ToString();
            result = result + " " + pinfo.ShippingAddress.LastName.ToString();
            result = result + " " + pinfo.ShippingAddress.Address1.ToString();
            result = result + " " + pinfo.ShippingAddress.Address2.ToString();
            result = result + " " + pinfo.ShippingAddress.City.ToString();
            result = result + " " + pinfo.ShippingAddress.StateProvince.ToString();
            result = result + " " + pinfo.ShippingAddress.ZipPostalCode.ToString();


            return result;
        }

        #region TotalAmountForGuestPayment
        public Order TotalAmountForGuestPayment(PaymentInfo paymentInfo, Customer customer)
        {
            ...
            if (!String.IsNullOrEmpty(processPaymentResult.Error))
            {
                logService.InsertLog(LogTypeEnum.OrderError, string.Format("Error while calculating  order total. {0}", processPaymentResult.Error), processPaymentResult.FullError);
            }
            return order;
        }

        public Order VendorBasedTotalAmountForGuestPayment(PaymentInfo paymentInfo, Customer customer, int vendorId)
        {
            Guid orderGuid = new Guid();
            ...
            try
            {
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    cart = shoppingCartService.GetCustomerShoppingCart(customer.CustomerId, ShoppingCartTypeEnum.ShoppingCart, vendorId);
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (shoppingCartRequiresShipping)
                    {
                        if (paymentInfo.ShippingAddress != null)
                        {
                            if (!CommonHelper.IsValidEmail(paymentInfo.ShippingAddress.Email))
                            {
                                //If not valid, try the customer email.
                                if (!CommonHelper.IsValidEmail(customer.Email)) { 
                                    //OK, there is a problem, there's really no email.
                                    throw new NopException("Email is not valid");
                                }
                            }
                        }
        ...
        public Order TotalAmountForPayment(PaymentInfo paymentInfo, Customer customer)
        {
            Guid orderGuid = new Guid();
        ...
        public Order VendorBasedTotalAmountForPayment(PaymentInfo paymentInfo, Customer customer, int vendorId)
        {
            Guid orderGuid = new Guid();
            Order order = new Order();
            var processPaymentResult = new ProcessPaymentResult();
            ...
            try
            {
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (!CommonHelper.IsValidEmail(paymentInfo.BillingAddress.Email))
                    {
                        if (!CommonHelper.IsValidEmail(NopContext.Current.User.Email))
                        {
                            throw new NopException("Email is not valid");
                        } else {
                            paymentInfo.BillingAddress.Email = NopContext.Current.User.Email;
                        }
                    }
                }
                ...
                if (!paymentInfo.IsRecurringPayment)
                {
                    ...
                    if (shoppingCartRequiresShipping)
                    {
                        ...
                        {
                            //Email Not Valid
                            //Look for new email address
                            if (!CommonHelper.IsValidEmail(NopContext.Current.User.Email))
                            {
                                //User Email not found throw error.
                                throw new NopException("Email is not valid");
                            }
                            else
                            {
                                //Email found
                                paymentInfo.ShippingAddress.Email = NopContext.Current.User.Email;
                            }
                        }
                    }
                }
            ...
            if (!String.IsNullOrEmpty(processPaymentResult.Error))
            {
                logService.InsertLog(LogTypeEnum.OrderError, string.Format("Error while Calculating vendor based  order total. {0}", processPaymentResult.Error), processPaymentResult.FullError);
            }
            return order;
        }
        ...
        /// <param name="orderNote">The customer's note to the vendor.</param>
        /// <param name="orderId">Order identifier</param>
        /// <returns>The error status, or String.Empty if no errors</returns>
        public string PlaceOrder(PaymentInfo paymentInfo, Customer customer,
            Guid orderGuid, int vendorId, string orderNote, out int orderId)
        {
            orderId = 0;
            var processPaymentResult = new ProcessPaymentResult();
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(processPaymentResult.Error))
                {
                    ...
                    using (var scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        //IoC.Resolve<IChargeService>().AddCharge(orderId, 0, order.CustomerId, order.OrderSubTotalDiscountExclTax * .05, true, 0, "Charge to Seller for Commission", 0, order.OrderProductVariants[0].ProductVariant.VendorId, DateTime.Now, DateTime.Now, DateTime.Now(), order.CustomerId, order.CustomerId);
                        ...
                        //notes, messages
                        InsertOrderNote(order.OrderId, string.Format("Order placed"), false, DateTime.UtcNow);
                        if (orderNote.Trim() != string.Empty)
                        {
                            InsertOrderNote(order.OrderId, orderNote, true, DateTime.UtcNow);
                        }
                        int orderPlacedStoreOwnerNotificationQueuedEmailId = messageService.SendOrderPlacedSellerNotification(order, localizationManager.DefaultAdminLanguage.LanguageId);
                        if (orderPlacedStoreOwnerNotificationQueuedEmailId > 0)
                        {
                            InsertOrderNote(order.OrderId, string.Format("\"Order placed\" email (to store Seller) has been queued. Queued email identifier: {0}.", orderPlacedStoreOwnerNotificationQueuedEmailId), false, DateTime.UtcNow);
                        }

                        int orderPlacedSewbieNotificationQueuedEmailId = messageService.SendOrderPlacedSewbieNotification(order, localizationManager.DefaultAdminLanguage.LanguageId);
                        if (orderPlacedSewbieNotificationQueuedEmailId > 0)
                        {
                            InsertOrderNote(order.OrderId, string.Format("\"Order placed\" email (to Sewbie) has been queued. Queued email identifier: {0}.", orderPlacedStoreOwnerNotificationQueuedEmailId), false, DateTime.UtcNow);
                        }
            ...
            if (!String.IsNullOrEmpty(processPaymentResult.Error))
            {
                ...
                logService.InsertLog(LogTypeEnum.OrderError, orderGuid.ToString(), string.Empty);
                logService.InsertLog(LogTypeEnum.OrderError, orderId.ToString(), string.Empty);
                logService.InsertLog(LogTypeEnum.OrderError, orderNote, string.Empty);
                logService.InsertLog(LogTypeEnum.OrderError, vendorId.ToString(), string.Empty);
            }
            return processPaymentResult.Error;
        }
