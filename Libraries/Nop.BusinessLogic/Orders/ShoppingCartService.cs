...
namespace NopSolutions.NopCommerce.BusinessLogic.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        /// <summary>
        /// Gets a VENDOR shopping cart by customer session GUId
        /// </summary>
        /// <param name="shoppingCartType">Shopping cart type</param>
        /// <param name="customerSessionGuid">The customer session identifier</param>
        /// <returns>Cart</returns>
        public ShoppingCart GetShoppingCartByCustomerSessionGuid(ShoppingCartTypeEnum shoppingCartType,
            Guid customerSessionGuid, int vendorId)
        {
            var query = from sci in _context.ShoppingCartItems
                        orderby sci.CreatedOn
                        where sci.ShoppingCartTypeId == (int)shoppingCartType && sci.CustomerSessionGuid == customerSessionGuid && sci.VendorID == vendorId
                        select sci;
            var scItems = query.ToList();
        ...
        /// <summary>
        /// Gets current user VENDOR shopping cart
        /// </summary>
        /// <param name="shoppingCartType">Shopping cart type</param>
        /// <returns>Cart</returns>
        public ShoppingCart GetCurrentShoppingCart(ShoppingCartTypeEnum shoppingCartType, int vendorId)
        {
            if (NopContext.Current.Session == null)
                return new ShoppingCart();
            var customerSessionGuid = NopContext.Current.Session.CustomerSessionGuid;
            return GetShoppingCartByCustomerSessionGuid(shoppingCartType, customerSessionGuid, vendorId);
        }
        ...
        /// <returns>Cart</returns>
        public ShoppingCart GetCustomerShoppingCart(int customerId,
            ShoppingCartTypeEnum shoppingCartType, int vendorId)
        {
            var customerSession = IoC.Resolve<ICustomerService>().GetCustomerSessionByCustomerId(customerId);
            if (customerSession == null)
                return new ShoppingCart();
            ...
            decimal customerEnteredPrice, int quantity)
        {
            ...
            ProductVariant pv = IoC.Resolve<IProductService>().GetProductVariantById(productVariantId); 
            ...
            if (shoppingCartItem != null)
            {
                ...
                if (warnings.Count == 0)
                {
                    ...
                    shoppingCartItem.VendorID = pv.VendorId;
